...
use rayon::prelude::*;
use self_encryption::{
    self, DataMap, decrypt_full_set, encrypt, EncryptedChunk, Error, Result, test_helpers,
};
use serde::Deserialize;
use xor_name::XorName;

use crate::rustgo::{allocate, deallocate, get_string, return_string};
...
pub extern fn self_encrypt(filepath_ptr: *mut c_char) -> () {
    ...
    //         Ok(_) => println!("{}", String::from_utf8(data).unwrap()),
...
//         let mut path = PathBuf::from(args.arg_target.clone().unwrap());
//         path.pop();
//         let chunks_path = path.to_str().unwrap().to_owned();
//         let stor = Arc::new(DiskBasedStorage { storage_path: chunks_path });
//         if let Ok(mut file) = File::open(args.arg_target.clone().unwrap()) {
//             let mut data = Vec::new();
